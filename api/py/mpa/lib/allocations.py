...
class OverheadAllocator:
    ...
    def calculate_pools(
        ...
        Returns:
            Dictionary with pool amounts and breakdown:
            - 'sga_pool': SG&A overhead
            - 'data_pool': Data infrastructure
            - 'workplace_pool': Workplace well-being
            - 'sga_from_pnl': SG&A portion from P&L
            - 'data_from_pnl': Data portion from P&L
            - 'workplace_from_pnl': Workplace portion from P&L
            - 'nil_excluded': NIL bucket total (excluded)
            - 'sga_from_cc': SG&A portion from cost centers
            - 'data_from_cc': Data portion from cost centers
        """
        # P&L buckets
        data_pnl = float(pnl_df[pnl_df['bucket'] == 'DATA']['amount'].sum()) if not pnl_df.empty else 0.0
        workplace_pnl = float(pnl_df[pnl_df['bucket'] == 'WORKPLACE']['amount'].sum()) if not pnl_df.empty else 0.0
        sga_pnl = float(pnl_df[pnl_df['bucket'] == 'SGA']['amount'].sum()) if not pnl_df.empty else 0.0
        nil_excluded = float(pnl_df[pnl_df['bucket'] == 'NIL']['amount'].sum()) if not pnl_df.empty else 0.0
        ...
        if include_cc_in_sga and not cost_centers_df.empty:
            if 'total_cost' in cost_centers_df.columns and 'pool' in cost_centers_df.columns:
                data_cc = float(cost_centers_df[cost_centers_df['pool'] == 'DATA']['total_cost'].sum())
                sga_cc = float(cost_centers_df[cost_centers_df['pool'] == 'SGA']['total_cost'].sum())
                data_pool += data_cc
                sga_pool += sga_cc
                data_from_cc = data_cc
                sga_from_cc = sga_cc
        ...
        return {
            ...
            'sga_from_pnl': sga_pnl,
            'data_from_pnl': data_pnl,
            'workplace_from_pnl': workplace_pnl,
            'nil_excluded': nil_excluded,
            'sga_from_cc': sga_from_cc,
...
def get_tagged_revenue(revenue_df: pd.DataFrame) -> Dict[str, float]:
    """
    Get revenue totals by allocation tag.

    Args:
        revenue_df: Revenue centers with allocation_tag and revenue

    Returns:
        Dictionary with:
        - 'total_revenue': Sum of all revenue
        - 'data_tagged_revenue': Sum of Data-tagged revenue
        - 'wellness_tagged_revenue': Sum of Wellness-tagged revenue
    """
    total = float(revenue_df['revenue'].sum())
    data_rev = float(revenue_df[revenue_df['allocation_tag'] == 'Data']['revenue'].sum())
    wellness_rev = float(revenue_df[revenue_df['allocation_tag'] == 'Wellness']['revenue'].sum())

    return {
        'total_revenue': total,
        'data_tagged_revenue': data_rev,
        'wellness_tagged_revenue': wellness_rev,
    }
